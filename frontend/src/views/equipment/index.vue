...
const auditDialog = ref(false); const auditForm = reactive({ id: '', auditStatus: '', auditorName: '' })
...
const doAudit = (row) => { auditForm.id = row.id; auditDialog.value = true }
const submitAudit = async () => {
  await request.put(`/equipment/repairs/${auditForm.id}/audit`, { auditStatus: auditForm.auditStatus, auditorName: auditForm.auditorName })
  ElMessage.success('审核完成'); auditDialog.value = false; repairDialog.value = false
}
const doStartRepair = async (row) => { await request.put(`/equipment/repairs/${row.id}/start`); ElMessage.success('开始维修') }
const doFinishRepair = (row) => { finishForm.id = row.id; finishDialog.value = true }
const submitFinish = async () => {
  ...
  ElMessage.success('维修完成'); finishDialog.value = false; repairDialog.value = false
}
...
<template>
  <div>
    ...
    <el-dialog v-model="eqDialog" title="器材信息" width="400px"><el-form label-width="80px">
      ...
      <template #footer><el-button @click="eqDialog = false">取消</el-button><el-button type="primary" @click="handleEqSubmit">保存</el-button></template>
    </el-form></el-dialog>

    <!-- Report dialog -->
    <el-dialog v-model="rptDialog" title="器材报修" width="400px"><el-form label-width="80px">
      ...
      <template #footer><el-button @click="rptDialog = false">取消</el-button><el-button type="danger" @click="doReport">提交报修</el-button></template>
    </el-form></el-dialog>
    ...
    <!-- Audit dialog -->
    <el-dialog v-model="auditDialog" title="审核报修" width="350px"><el-form label-width="80px">
      <el-form-item label="审核"><el-radio-group v-model="auditForm.auditStatus"><el-radio value="APPROVED">通过</el-radio><el-radio value="REJECTED">驳回</el-radio></el-radio-group></el-form-item>
      <el-form-item label="审核人"><el-input v-model="auditForm.auditorName"/></el-form-item>
      <template #footer><el-button @click="auditDialog = false">取消</el-button><el-button type="primary" @click="submitAudit">确认</el-button></template>
    </el-form></el-dialog>

    <!-- Finish repair dialog -->
    <el-dialog v-model="finishDialog" title="维修完成" width="350px"><el-form label-width="80px">
      ...
      <template #footer><el-button @click="finishDialog = false">取消</el-button><el-button type="primary" @click="submitFinish">确认</el-button></template>
    </el-form></el-dialog>
  </div>
</template>
